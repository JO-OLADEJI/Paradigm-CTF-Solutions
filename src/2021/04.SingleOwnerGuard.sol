...
contract SingleOwnerGuard is Guard {
    ...
    function cleanup() external {
        ...
        // @audit-info: I smell trouble... just saying
        selfdestruct(owner());
    }
    ...
    function isAllowed(
        ...
        // owner can execute any operation
        if (who == owner()) return (NO_ERROR, 1);

        // @audit-info: operations registered would not throw an error even if `isAllowed()` is not called by `owner()`
